      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {content.map((category, index) => {
          ...
          return (
            <div
              key={index}
              className="panel rounded-lg border-l-4 border-teal-700 bg-neutral-300 p-4 dark:border-teal-300 dark:bg-neutral-700"
            >
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {category.skills.map((skill, skillIndex) => (
                  <span
                    key={skillIndex}
                    className="item xl:text-md rounded-full bg-teal-200 px-3 py-1 text-xs font-medium text-teal-900 transition-colors hover:bg-teal-400 md:text-sm dark:bg-teal-800 dark:text-teal-100 dark:hover:bg-teal-600"
                  >
                    {skill}
                  </span>
                ))}
